...
godot::Interactable::Interactable(Vector3 position, int r, int c, bool flat_topped, float outer_size, float inner_size, float height, uint8_t type) :
		Tile(position, r, c, flat_topped, outer_size, inner_size, height, type) {
}
...
void godot::Interactable::TileSteppedOnEvent() {
  ...
  if(SteppedOnTileCallables.size() > 0) {
    for(int i = 0; i < SteppedOnTileCallables.size(); i++) {
      UtilityFunctions::print("Calling Event ", i);
